...
function displayRecommendation(result, type) {
    ...
    if (type === 'drehzahl' && result >= warningThresholds.drehzahl) {
        ...
        recommendation = `${type.charAt(0).toUpperCase() + type.slice(1)} is optimal: ${result}`;
    }
...
function kapazitaet() {
    ...
    if (!isNaN(cRate) && !isNaN(strom)) {
        ...
        document.getElementById("endAh").innerHTML = `Kapazität: ${kapazitaet.toFixed(2)}Ah`;
    } else {
...
function CRate() {
    ...
    if (!isNaN(strom) && !isNaN(kapazitaet)) {
        ...
        document.getElementById("endCRate").innerHTML = `C-Rate: ${cRate.toFixed(2)}`;
    } else {
